...
async function refreshSEAM() {
    try {
        const [live, tracks, forecasts, manifolds, aggregates] = await Promise.all([
        ...
        document.getElementById("summary").innerText = "Feed unavailable: " + err.message;
    }
}

function renderSummary(state) {
    document.getElementById("summary").innerText =
        `${state.timestamp_utc || ""} | events=${state.events.length} | tracks=${state.tracks.length} | forecasts=${state.forecasts.length} | manifolds=${state.manifolds.length}`;
}
